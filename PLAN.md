...
2. `git init && git remote add origin https://github.com/gitychzh/openclaw_improve_self.git`
3. 创建所有目录和文件（scripts/, templates/, memory/, state/, tests/）
4. 写 .gitignore
5. 首次 commit + push

### Phase 2: 健康检查
1. 写 scripts/health-check.sh
2. 写 tests/health-check.test.sh
...
3. 配置 git pre-commit hook（可选）
4. 测试回滚流程

### Phase 5: 集成测试
1. 做一次 L3 小改动（改 MEMORY.md）→ health check → commit → push
2. 做一次 L2 小改动（改 TOOLS.md）→ health check → commit → push
3. 模拟 health check 失败 → 验证自动回滚
4. 通过 ACP 让 CC 做一次小优化 → 验证全流程
...
- [ ] Boss 张批准本规划
- [ ] 远程仓库已创建（gitychzh/openclaw_improve_self）
- [ ] 开始执行 Phase 1
